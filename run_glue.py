"""
This script is adapted from https://github.com/huggingface/transformers/blob/main/examples/pytorch/text-classification/run_glue.py
"""
...
from models import RevBertForSequenceClassification
...
class ModelArguments:
    ...
    f_arch: ChoiceEnum(["layer", "adapter", "attn"]) = field(
        default="layer",
        metadata={"help": "what is the architecture for F, choices=[layer, adapter, attn]"}
    )
...
def main():
    ...
    sequence_classification_model = RevBertForSequenceClassification
    ...
    # For reversible network, we need some parameters tunable before the reversible layers. Only in this way,
    # the back-propagation can work properly.
    # That's why I set layernorm as tunable here. But it is not updated,
    # I set the grad as None after .backward(). Check CustomTrainer for this.
    if model_args.adapter_bottleneck_dim > 0:
        for param in model.parameters():
            param.requires_grad = False
        non_freeze_sets = ["adapter", "classifier"]
        if model_args.freeze_irreversible_layers:  # The third setting in Figure 7
            non_freeze_sets.append(f"encoder.layer.{config.num_hidden_layers - model_args.num_rev_layers - 1}."
                                   f"output.LayerNorm")
        else:
            non_freeze_sets.append("embeddings.LayerNorm")
    ...
    if training_args.do_train:
        ...
        metrics["peak mem (G)"] = torch.cuda.max_memory_allocated() / (1024 * 1024 * 1000)
        metrics["activation mem (G)"] = (torch.cuda.max_memory_allocated() - torch.cuda.memory_allocated()) / (1024 * 1024 * 1000)
